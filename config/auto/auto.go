...
import (
	"fmt"
	"gitlab.com/devpro_studio/Paranoia/cache/etcd"
	"gitlab.com/devpro_studio/Paranoia/cache/memcached"
	...
	"gitlab.com/devpro_studio/Paranoia/client/kafka-client"
		...
		for _, mod := range modules {
			...
			case "cache":
				...
				case "memcached":
					...
					t.app.PushCache(memcached.NewMemcached(name.(string), cfg))
				...
				case "memory":
					...
					t.app.PushCache(memory.NewMemory(name.(string), cfg))
				...
				case "redis":
					...
					t.app.PushCache(redis.NewRedis(name.(string), cfg))
				...
				case "etcd":
					...
					t.app.PushCache(etcd.NewEtcd(name.(string), cfg))
			...
			case "client":
				...
				case "http":
					...
					t.app.PushClient(http_client.NewHTTPClient(name.(string), cfg))

				case "kafka":
					cfg := kafka_client.KafkaClientConfig{}
					err = mod.Scan(&cfg)
					if err != nil {
						return err
					}

					t.app.PushClient(kafka_client.NewKafkaClient(name.(string), cfg))

				case "grpc":
					cfg := grpc_client.GrpcClientConfig{}
					err = mod.Scan(&cfg)
					if err != nil {
						return err
					}

					t.app.PushClient(grpc_client.NewGrpcClient(name.(string), cfg))
