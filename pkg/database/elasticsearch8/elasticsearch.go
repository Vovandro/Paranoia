...
func New(name string) *ElasticSearch {
	return &ElasticSearch{name: name}
}
...
func (t *ElasticSearch) Init(cfg map[string]interface{}) error {
	...
	t.client, err = es8.NewClient(esCfg)
	if err != nil {
		return err
	}
...
func (t *ElasticSearch) Stop() error {
	return nil
}

func (t *ElasticSearch) Name() string {
	return t.name
}
...
func (t *ElasticSearch) Get(ctx context.Context, index string, id string) (NoSQLRow, error) {
	...
	if res != nil && res.Body != nil {
		// Do not close here; reader will consume and close via Scan/Close
	}
	if res.IsError() {
		data, _ := io.ReadAll(res.Body)
		_ = res.Body.Close()
...
func (t *ElasticSearch) Search(ctx context.Context, index []string, query *types.Query, from, size int) (NoSQLRows, error) {
	defer func(s time.Time) { t.timeCounter.Record(context.Background(), time.Since(s).Milliseconds()) }(time.Now())
	t.counter.Add(context.Background(), 1)
